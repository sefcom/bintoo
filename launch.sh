...
docker run -i --rm \
    ...
    -v $PWD/out-$O-$WORKER/O$O/Packages:/shared/O$O/Packages \
    --privileged=true \
    bintoo \
...
cp -v $PWD/out-$O-$WORKER/O$O/$TEMP_PACKAGES_FILENAME $PWD/out-$O-$WORKER/O$O/Packages
cp -v $PWD/out-$O-$WORKER/O$O/$TEMP_PACKAGES_FILENAME $PWD/out-$O-$WORKER/O$O/Packages_merged
rm -f $TEMP_PACKAGES
